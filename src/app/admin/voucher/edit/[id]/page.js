...
export default function EditVoucherPage({ params }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/voucher/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setForm({
          kode: data.kode || "",
          diskon: data.diskon || "",
          tipe: data.tipe || "persen",
          minBelanja: data.minBelanja || "",
          maxDiskon: data.maxDiskon || "",
          kuota: data.kuota || "1",
          aktif: data.aktif !== false,
          expiredAt: data.expiredAt
            ? new Date(data.expiredAt).toISOString().slice(0, 16)
            : "",
        });
        setFetching(false);
      });
  ...
  const handleSubmit = async () => {
    ...
    if (!res.ok) {
      setError(data.error || "Gagal mengupdate voucher");
      setLoading(false);
      return;
    }
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center h-40">
        <p className="text-gray-400 text-sm">Memuat data voucher...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Edit Voucher</h1>
